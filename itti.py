...
class GaussianPyramid:
    ...
    def __get_colors(self, b, g, r, i, amax):
        ...
        nb, ng, nr = list(map(lambda x, y, z: max(x - (y + z) / 2., 0.), [b, g, r], [g, b, r], [r, g, b]))  # 两两相减形成3个色差图
...
class ConspicuityMap:
    ...
    def __make_conspicuity_map(self, srcs):
        intensity = self.__scale_add(list(map(utils.normalize, srcs['intensity'])))  # 亮度累加
        ...
        for key in srcs['orientations'].keys():
            orientation += self.__scale_add(list(map(utils.normalize, srcs['orientations'][key])))  # 方向累加爱
